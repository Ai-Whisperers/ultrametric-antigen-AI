...
**Doc-Type:** Technical Reference · Version 1.1 · Updated 2025-12-27 · AI Whisperers
...
## Metrics Prioritization Matrix

### Primary Metrics (Stored in Checkpoints)

| Priority | Metric | Formula | Target | Source |
|:--------:|--------|---------|--------|--------|
| **1** | Coverage | `unique_ops / 19,683 × 100%` | 100% | `src/utils/metrics.py:19-39` |
| **2** | Hierarchy_B | `spearmanr(valuation, radius_B)` | -0.83 to -1.0 | `analyze_all_checkpoints.py:93` |
| **3** | Richness | `Σ var(radii[v]) / 10` for v=0..9 | Higher = better | `analyze_all_checkpoints.py:96-101` |
| **4** | r_v0 | Mean radius at valuation=0 | ~0.85-0.95 | Radial separation |
| **5** | r_v9 | Mean radius at valuation≥8 | ~0.15-0.20 | Radial separation |

### Derived Metrics (Training/Monitoring)

| Metric | Formula | Purpose | Source |
|--------|---------|---------|--------|
| Q (Structure) | `dist_corr + 1.5 × \|hierarchy\|` | Homeostatic trigger | `src/models/homeostasis.py:45-52` |
| Ranking Corr | Triplet concordance (p-adic vs hyperbolic) | Alternative hierarchy | `src/core/metrics.py:58-175` |
| Radial Error | `abs(actual_r - target_r).mean()` | Training signal | `src/losses/radial_stratification.py:148` |
| Diversity | Jaccard distance VAE-A vs VAE-B | Encoder differentiation | `src/utils/metrics.py:81-119` |

### Metric Formulas (Quick Reference)

```python
# COVERAGE (Priority 1)
coverage = len(unique(round(samples))) / 19683 * 100

# HIERARCHY (Priority 2) - target: negative, ceiling: -0.8321
hierarchy = spearmanr(valuations, radii)[0]

# RICHNESS (Priority 3) - higher = more geometric diversity
richness = sum(radii[v].var() for v in range(10)) / 10

# Q METRIC - homeostatic freeze/unfreeze decisions
Q = dist_corr + 1.5 * abs(hierarchy)

# TARGET RADIUS - expected radial position per valuation
target_r = 0.85 - (valuation / 9) * 0.75  # v=0→0.85, v=9→0.10
```
...
|------|---------|---------|
| 2025-12-27 | 1.1 | Added Metrics Prioritization Matrix with formulas, sources, targets |
