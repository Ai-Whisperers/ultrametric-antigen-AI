...
from utils.plotting import setup_pitch_style, PALETTE, save_figure, add_goldilocks_zones
...
def create_goldilocks_gauge():
    ...
    # Title
    ax.text(0, 1.25, 'The Goldilocks Zone of Autoimmunity',
            fontsize=22, fontweight='bold', ha='center', color=PALETTE['text'])
    ax.text(0, 1.1, 'Optimal Perturbation Range for Immune Recognition',
            fontsize=12, ha='center', color=PALETTE['text_light'])

    # Create semi-circular gauge
    # Zone 1: Too little (<15%) - Blue
    wedge1 = Wedge((0, 0), 1, 126, 180, width=0.3,
                   facecolor=PALETTE['goldilocks_low'], edgecolor='white', linewidth=2)
    ax.add_patch(wedge1)

    # Zone 2: Goldilocks (15-30%) - Yellow/Gold
    wedge2 = Wedge((0, 0), 1, 72, 126, width=0.3,
                   facecolor='#FFD700', edgecolor='white', linewidth=2)
    ax.add_patch(wedge2)

    # Zone 3: Too much (>30%) - Red
    wedge3 = Wedge((0, 0), 1, 0, 72, width=0.3,
                   facecolor=PALETTE['goldilocks_high'], edgecolor='white', linewidth=2)
    ax.add_patch(wedge3)

    # Zone labels
    ax.text(-0.85, 0.5, 'Too\nLittle', fontsize=11, ha='center', va='center',
            color=PALETTE['risk_protective'], fontweight='bold')
    ax.text(0, 0.85, 'Goldilocks\nZone', fontsize=12, ha='center', va='center',
            color='#B8860B', fontweight='bold')
    ax.text(0.85, 0.5, 'Too\nMuch', fontsize=11, ha='center', va='center',
            color=PALETTE['risk_high'], fontweight='bold')
    ...
    outcomes = [
        {'x': -0.9, 'text': 'Recognized as\n"SELF"', 'sub': 'No immune response',
         'color': PALETTE['goldilocks_low']},
        {'x': 0, 'text': 'Recognized as\n"MODIFIED SELF"', 'sub': 'T-cell activation\nAutoimmunity',
         'color': '#FFD700'},
        {'x': 0.9, 'text': 'Recognized as\n"FOREIGN"', 'sub': 'Rapid clearance',
         'color': PALETTE['goldilocks_high']},
    ]

    for outcome in outcomes:
        ax.text(outcome['x'], -0.15, outcome['text'], fontsize=10, ha='center',
                va='top', color=outcome['color'], fontweight='bold')
        ax.text(outcome['x'], -0.4, outcome['sub'], fontsize=9, ha='center',
                va='top', color=PALETTE['text_light'])

    # Key epitopes in Goldilocks zone
    epitopes_box = FancyBboxPatch(
        (-1.3, -0.85), 2.6, 0.35,
        boxstyle="round,pad=0.02,rounding_size=0.1",
        facecolor='#FFFDE7', edgecolor='#FFD700', linewidth=2
    )
    ax.add_patch(epitopes_box)
    ax.text(0, -0.67, 'Key Epitopes in Goldilocks Zone: VIM_R71 (19%) • FGA_R38 (24.5%) • FLG_CCP (21.2%)',
            fontsize=10, ha='center', va='center', color='#B8860B')
...
def create_epitope_shift_chart():
    ...
    # Zone labels
    ax.text(len(epitopes) - 0.5, 22.5, 'GOLDILOCKS\nZONE', fontsize=10,
            ha='right', va='center', color='#B8860B', fontweight='bold')
    ...
    for i, (shift, acpa) in enumerate(zip(shifts, acpa_values)):
        if acpa > 50:
            ax.scatter(i, shift + 1.5, s=acpa * 2, c='red', marker='v',
    ...
    legend_elements = [
        ...
        Line2D([0], [0], marker='v', color='w', markerfacecolor='red',
               markersize=10, label='High ACPA (>50%)'),
    ]
...
def create_statistics_summary():
    ...
    # Title
    ax.text(5, 5.5, 'Statistical Evidence for Goldilocks Hypothesis',
            fontsize=18, fontweight='bold', ha='center', color=PALETTE['text'])

    # Create comparison boxes
    metrics = [
        {'name': 'Centroid Shift', 'key': 'centroid_shift',
        ...
        {'name': 'JS Divergence', 'key': 'js_divergence',
        ...
        {'name': 'Entropy Change', 'key': 'entropy_change',
    ...
    for i, metric in enumerate(metrics):
        ...
        box = FancyBboxPatch(
            (x_base - 1, 2), 2.5, 2.5,
            boxstyle="round,pad=0.02,rounding_size=0.1",
            facecolor='white', edgecolor=PALETTE['primary'], linewidth=2
        )
        ax.add_patch(box)

        # Metric name
        ax.text(x_base + 0.25, 4.2, metric['name'], fontsize=12, fontweight='bold',
                ha='center', va='center', color=PALETTE['text'])

        # Values
        ax.text(x_base + 0.25, 3.5, f"Immunodominant: {metric['imm']}", fontsize=10,
                ha='center', va='center', color=PALETTE['immunodominant'])
        ax.text(x_base + 0.25, 3.0, f"Silent: {metric['sil']}", fontsize=10,
                ha='center', va='center', color=PALETTE['silent'])
        ...
        ax.text(x_base + 0.25, 2.4, f"{p_text} {stars}", fontsize=11,
                ha='center', va='center', color=p_color, fontweight='bold')

    # Summary statement
    ax.text(5, 1.2, 'All three metrics show statistically significant differences (p < 0.05)',
            fontsize=12, ha='center', va='center', color=PALETTE['text'])
    ax.text(5, 0.7, 'Immunodominant epitopes cluster in the 15-30% "modified self" zone',
            fontsize=11, ha='center', va='center', color=PALETTE['text_light'], style='italic')
