...
def plot_model_prediction_accuracy(data: Dict, output_dir: Path):
    ...
    ax1.hist(imm, bins=bins, alpha=0.7, color=COLORS['immunodominant'],
             label=f'Immunodominant (n={len(imm)})', edgecolor='black')
    ax1.hist(sil, bins=bins, alpha=0.7, color=COLORS['silent'],
             label=f'Silent (n={len(sil)})', edgecolor='black')

    # Add Goldilocks zone
    ax1.axvspan(GOLDILOCKS_ALPHA, GOLDILOCKS_BETA, alpha=0.2, color=COLORS['goldilocks'],
                label='Goldilocks Zone')

    ax1.set_xlabel('Hyperbolic Entropy Change (ΔH)', fontsize=12)
    ax1.set_ylabel('Count', fontsize=12)
    ax1.set_title('A. Entropy Change Distribution\nModel Predictions', fontsize=14, fontweight='bold')
    ax1.legend(fontsize=10)
    ax1.grid(axis='y', alpha=0.3)

    # Statistics
    t_stat, p_val = stats.ttest_ind(imm, sil)
    ax1.text(0.02, 0.98, f't = {t_stat:.2f}\np = {p_val:.4f}',
    ...
    colors = [COLORS['immunodominant'] if imm else COLORS['silent']
              for imm in df['immunodominant']]

    ax2.scatter(df['entropy_change'], df['acpa_reactivity'], c=colors, s=150,
                edgecolor='black', linewidth=1.5, alpha=0.8)

    # Add labels for key epitopes
    for _, row in df.iterrows():
        if row['acpa_reactivity'] > 0.7 or row['entropy_change'] > 0.04:
            ax2.annotate(row['epitope_id'], (row['entropy_change'], row['acpa_reactivity']),
    ...
    ax2.plot(x_line, slope * x_line + intercept, '--', color='gray', alpha=0.7,
             label=f'r = {r_val:.3f}, p = {p_val:.3f}')

    ax2.axvspan(GOLDILOCKS_ALPHA, GOLDILOCKS_BETA, alpha=0.15, color=COLORS['goldilocks'])

    ax2.set_xlabel('Entropy Change (Model Prediction)', fontsize=12)
    ax2.set_ylabel('ACPA Reactivity (Clinical)', fontsize=12)
    ax2.set_title('B. Model Prediction vs Clinical Outcome\nEntropy-ACPA Correlation', fontsize=14, fontweight='bold')
    ...
    ax3.plot(fpr, tpr, color=COLORS['model'], lw=3,
    ...
    ax3.plot(fpr_g, tpr_g, color=COLORS['alphafold'], lw=3,
             label=f'Goldilocks Zone (AUC = {roc_auc_g:.3f})')

    ax3.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
    ax3.set_xlabel('False Positive Rate', fontsize=12)
    ax3.set_ylabel('True Positive Rate', fontsize=12)
    ax3.set_title('C. ROC Curve: Predicting Immunodominance',
    ...
    ax4.axhline(y=0.8, color='red', linestyle='--', alpha=0.5, label='Large effect (d=0.8)')
    ax4.axhline(y=0.5, color='orange', linestyle='--', alpha=0.5, label='Medium effect (d=0.5)')

    ax4.set_ylabel("Cohen's d (Effect Size)", fontsize=12)
    ax4.set_title('D. Model Feature Discrimination Power\nImmunodom. vs Silent Epitopes',
                  fontsize=14, fontweight='bold')
...
def plot_model_vs_alphafold(data: Dict, output_dir: Path):
    ...
    if matched_data:
        ...
        # Scatter plot
        sc = ax1.scatter(matched_df['model_entropy'], matched_df['af3_delta_iptm'],
                         c=matched_df['acpa'], cmap='RdYlBu_r', s=300,
                         edgecolor='black', linewidth=2)

        # Labels
        for _, row in matched_df.iterrows():
            ax1.annotate(row['epitope'].upper(),
    ...
    ax1.set_title('A. Model vs AlphaFold 3 Cross-Validation\nEntropy Change Predicts HLA Binding',
                  fontsize=14, fontweight='bold')
    ax1.axvspan(GOLDILOCKS_ALPHA, GOLDILOCKS_BETA, alpha=0.15, color=COLORS['goldilocks'],
                label='Goldilocks Zone')
    ...
    colors = [COLORS['model'], COLORS['alphafold'], COLORS['proteome'], COLORS['immunodominant']]
...
def plot_proteome_landscape(data: Dict, output_dir: Path):
    ...
    if risk_dist:
        ...
        wedges, texts, autotexts = ax1.pie(ordered_sizes, labels=ordered_labels, colors=ordered_colors,
        ...
        ax1.set_title(f'A. Proteome-Wide Risk Distribution\n(n={proteome_stats.get("total_sites", 0):,} arginine sites)',
                      fontsize=14, fontweight='bold')
    ...
    if high_risk is not None and 'entropy_change' in high_risk.columns:
        ...
        ax2.hist(high_risk['entropy_change'], bins=50, color=COLORS['high_risk'],
                 alpha=0.7, edgecolor='black')

        ax2.axvspan(GOLDILOCKS_ALPHA, GOLDILOCKS_BETA, alpha=0.2, color=COLORS['goldilocks'],
                    label='Goldilocks Zone')

        in_zone = ((high_risk['entropy_change'] >= GOLDILOCKS_ALPHA) &
                   (high_risk['entropy_change'] <= GOLDILOCKS_BETA)).sum()
        pct_in_zone = 100 * in_zone / len(high_risk)

        ax2.axvline(x=high_risk['entropy_change'].median(), color='red',
                    linestyle='--', label=f"Median: {high_risk['entropy_change'].median():.3f}")

        ax2.set_xlabel('Entropy Change (ΔH)', fontsize=12)
        ax2.set_ylabel('Count', fontsize=12)
        ax2.set_title(f'B. High-Risk Candidates Entropy Distribution\n{pct_in_zone:.1f}% in Goldilocks Zone',
                      fontsize=14, fontweight='bold')
        ax2.legend(fontsize=10)
    ...
    if high_risk is not None and 'gene_name' in high_risk.columns:
        ...
        bars = ax3.barh(range(len(gene_counts)), gene_counts.values, color=COLORS['proteome'],
                        edgecolor='black')
        ...
        ax3.set_title('C. Top 15 Proteins with High-Risk Sites\nPotential Novel Autoantigens',
                      fontsize=14, fontweight='bold')
    ...
    if high_risk is not None:
        ...
        categories = ['Known RA\nAutoantigens', 'In High-Risk\nPredictions', 'Novel High-Risk\nProteins']
        values = [len(known_antigens), known_in_high_risk, total_proteins - known_in_high_risk]
...
def plot_comprehensive_dashboard(data: Dict, output_dir: Path):
    ...
    └─────────────────────────────────────────────────────────────────────────────┘
    """
    ax_pipeline.text(0.5, 0.5, pipeline_text, transform=ax_pipeline.transAxes,
                     fontsize=10, fontfamily='monospace', ha='center', va='center',
                     bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
    ax_pipeline.set_title('A. 3-Adic Codon Encoder Model Pipeline',
                          fontsize=14, fontweight='bold')
    ...
    for i, (num, label, color) in enumerate(key_numbers):
        ...
        ax_numbers.text(x, y, num, fontsize=18, fontweight='bold', color=color,
                        transform=ax_numbers.transAxes)
        ax_numbers.text(x, y - 0.1, label, fontsize=10,
                        transform=ax_numbers.transAxes)
    ...
    if not df_epitopes.empty:
        ...
        ax_scatter.scatter(df_epitopes['entropy_change'], df_epitopes['acpa_reactivity'],
                           c=colors, s=sizes, edgecolor='black', linewidth=1.5, alpha=0.8)

        ax_scatter.axvspan(GOLDILOCKS_ALPHA, GOLDILOCKS_BETA, alpha=0.2, color=COLORS['goldilocks'])

        # Add decision boundary
        ax_scatter.axhline(y=0.3, color='gray', linestyle='--', alpha=0.5,
    ...
    ax_scatter.set_title('C. Model Predictions vs Clinical Outcomes\n(Size = ACPA intensity)',
                         fontsize=14, fontweight='bold')
    ax_scatter.legend([mpatches.Patch(color=COLORS['immunodominant']),
                       mpatches.Patch(color=COLORS['silent'])],
                      ['Immunodominant', 'Silent'], fontsize=10)
    ...
    if af3_binding:
        ...
        if epitope_summary:
            ...
            ax_af3.scatter(entropy, delta_iptm, s=250, c=COLORS['alphafold'],
                           edgecolor='black', linewidth=2)

            for e, ent, d in zip(epitopes, entropy, delta_iptm):
                ax_af3.annotate(e.upper(), (ent, d), xytext=(5, 5),
                                textcoords='offset points', fontsize=10, fontweight='bold')

            ax_af3.axvspan(GOLDILOCKS_ALPHA, GOLDILOCKS_BETA, alpha=0.15, color=COLORS['goldilocks'])

    ax_af3.set_xlabel('Model: Entropy Change', fontsize=12)
    ax_af3.set_ylabel('AlphaFold3: Δ iPTM', fontsize=12)
    ax_af3.set_title('D. AlphaFold 3 Structural Validation\nModel Predicts Binding Improvement',
                     fontsize=14, fontweight='bold')
    ...
    table = ax_table.table(cellText=table_data[1:], colLabels=table_data[0],
                           ...
                           colColours=[COLORS['model'], COLORS['model'],
                                       COLORS['alphafold'], COLORS['goldilocks']])
    table.auto_set_font_size(False)
    table.set_fontsize(10)
    table.scale(1.2, 1.8)

    ax_table.set_title('E. Cross-Validation Summary Table',
...
"""
    ...
    ax_conclusion.text(0.1, 0.9, conclusion_text, transform=ax_conclusion.transAxes,
                       fontsize=11, fontfamily='monospace', va='top',
                       bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.3))
    ax_conclusion.set_title('F. Key Conclusions', fontsize=14, fontweight='bold')

    # Main title
    fig.suptitle('3-Adic Codon Encoder: Model Validation Dashboard\n'
...
def plot_feature_importance(data: Dict, output_dir: Path):
    ...
    ax1.set_title('A. Feature Correlation with Immunodominance\nPositive = Predicts Immunogenic',
                  fontsize=14, fontweight='bold')
    ...
    if not df.empty and 'entropy_change' in df.columns:
        ...
        df['combined_score'] = (
            (df['entropy_change'] - df['entropy_change'].min()) /
        ...
        bp = ax2.boxplot([imm_scores, sil_scores],
    ...
    ax2.set_title('B. Model Score Distribution\nImmunodom. vs Silent Epitopes',
                  fontsize=14, fontweight='bold')
    ax2.grid(axis='y', alpha=0.3)
