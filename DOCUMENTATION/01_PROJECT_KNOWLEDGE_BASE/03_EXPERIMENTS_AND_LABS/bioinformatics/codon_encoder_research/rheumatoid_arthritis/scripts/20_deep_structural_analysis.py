    ...
    for i, res in enumerate(disorder_profile):
        if res['is_disordered']:
            if current_region is None:
                current_region = {'start': res['resnum'], 'end': res['resnum'], 'residues': [res]}
            else:
...
def generate_visualizations(all_analyses: List[Dict], output_dir: Path):
    ...
    ax.hist(imm_plddt, bins=10, alpha=0.6, color='#e53935', label=f'Immunodominant (n={len(imm_plddt)})', density=True)
    ax.hist(silent_plddt, bins=10, alpha=0.6, color='#1e88e5', label=f'Silent (n={len(silent_plddt)})', density=True)
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ax.scatter(row['plddt'], row['entropy_change'],
    ...
    ax.set_title('Entropy Change vs Structure Confidence\n(squares = disordered/flexible)', fontsize=12, fontweight='bold')
    ax.grid(True, alpha=0.3)

    # Add legend
    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#e53935', markersize=10, label='Immunodominant'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#1e88e5', markersize=10, label='Silent'),
        Line2D([0], [0], marker='s', color='w', markerfacecolor='gray', markersize=10, label='Disordered/Flexible'),
    ]
    ...
    ax = axes[1]
    summary_data = df.groupby('disorder_class').agg({
...
def main():
    ...
    sites_by_protein = sites_df.groupby('epitope_id').apply(
        lambda x: x.iloc[0]['epitope_id'].split('_')[0]
    ).to_dict()
    ...
    if all_sites:
        ...
        # Fisher's exact test
        contingency = [[imm_disordered, imm_total - imm_disordered],
