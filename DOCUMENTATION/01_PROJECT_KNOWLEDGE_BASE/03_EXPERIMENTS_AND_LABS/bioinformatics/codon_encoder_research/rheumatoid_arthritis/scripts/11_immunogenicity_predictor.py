...
def extract_features(epitope: dict, encoder, device: str = 'cpu') -> Optional[Dict]:
    ...
    for i, emb in enumerate(embeddings):
        ...
        if np.any(other_mask):
            ...
            ).item() for other in other_embeddings]
...
def train_evaluate_models(X: np.ndarray, y: np.ndarray,
    ...
    lopo_results = {name: {'y_true': [], 'y_pred': [], 'y_prob': []}
                    for name in models}
    ...
    skf_results = {name: {'y_true': [], 'y_pred': [], 'y_prob': []}
                   for name in models}
...
def plot_calibration(results: Dict, output_dir: Path):
    ...
    for cv_type, ax in zip(['lopo', 'skf'], axes):
        ...
        for key, res in results['cv_results'].items():
            ...
            ax.plot(prob_pred, prob_true, 's-', color=colors.get(model_name, 'gray'),
...
def plot_metrics_comparison(results: Dict, output_dir: Path):
    ...
    for cv_type in cv_types:
        for model in models:
            ...
            ax.bar(x + offset * width, values, width, label=labels[key],
