...
# Common English words + common programming terms to ignore
COMMON_WORDS = set(
    """
        ...
        color face wood main enough plain girl usual young ready above ever
        red list though feel talk bird soon body dog family direct pose leave
        ...
        plan figure star box noun field rest correct able pound done beauty
        ...
        ran check game shape equate hot miss brought heat snow tire bring yes
        ...
        art subject region energy hunt probable bed brother egg ride cell
        believe fraction forest sit race window store summer train sleep prove
        lone leg exercise wall catch mount wish sky board joy winter sat
        written wild instrument kept glass grass cow job edge sign visit past
        ...
        finger industry value fight lie beat excite natural view sense ear
        else quite broke case middle kill son lake moment scale loud spring
        ...
        suggest clean break lady yard rise bad blow oil blood touch grew cent
        mix team wire cost lost brown wear garden equal sent choose fell fit
        ...
        party supply bone rail imagine provide agree thus capital won't chair
        danger fruit rich thick soldier process operate guess necessary sharp
        wing create neighbor wash bat rather crowd corn compare poem string
        ...
        gather shop stretch throw shine property column wrong gray repeat
        ...
        mass card band rope slip win dream evening condition feed tool total
        basic smell valley nor double seat arrive master track parent shore
        ...
        quotient teeth shell neck def return class import from as in is None
        True False try except finally raise assert lambda yield await async
        int float str bool list dict set tuple bytes args kwargs print len
        range open input type id hash abs min max sum sorted map filter zip
        enumerate tensor array numpy torch pandas dataframe series variable
        function method parameter argument module package library
    """.split()
)
...
def scan_codebase():
    if not SETTINGS_FILE.exists():
        ...
        try:
            with open(SETTINGS_FILE, "r") as f:
                data = json.load(f)
                existing_words = set(data.get("cSpell.words", []))
        except Exception:
            existing_words = set()

    print(f"Known dictionary words: {len(existing_words)}")

    found_unknowns = Counter()
    scanned_files = 0
    skipped_files = 0

    # Pre-compile regex for performance
    # 1. Split by non-alpha
    ...
    for root, dirs, files in os.walk(PROJECT_ROOT):
        ...
        for file in files:
            if file.endswith((".py", ".md", ".json", ".ts", ".tsx", ".js")):
                path = Path(root) / file

                # Strict size limit 200KB for speed
    ...
    print(f"Scanned {scanned_files} files. Skipped {skipped_files} large files.")
    print(f"Found {len(found_unknowns)} unique unknown words.")


def _scan_file(path, splitter, camel, found_unknowns, existing_words, common_words):
    try:
            ...
            for line in f:
                ...
                for token in tokens:
                    ...
                    if camel.search(token):
                        sub_tokens = camel.sub(r"\1 \2", token).split()
                    else:
                        sub_tokens = [token]
                    ...
                    for w in sub_tokens:
                        ...
                        if w_lower in common_words:
                            continue
                        if w_lower in existing_words:
                            continue
                        found_unknowns[w_lower] += 1
    except Exception:
        pass
