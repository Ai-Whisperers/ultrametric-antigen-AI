...
class PersistenceDiagram:
    ...
    def wasserstein_distance(
        self, other: "PersistenceDiagram", p: int = 2
    ) -> float:
        ...
        for i in range(n):
            for j in range(m):
                cost[i, j] = (
                    abs(self.birth[i] - other.birth[j]) ** p
                    + abs(self.death[i] - other.death[j]) ** p
                ) ** (1 / p)
...
class RipsFiltration:
    ...
    def _build_gudhi(self, points: np.ndarray) -> TopologicalFingerprint:
        ...
        for dim in range(self.max_dimension + 1):
            ...
            dim_pairs = [
                (b, d)
                for (b, d) in simplex_tree.persistence()
                if b[0] == dim
            ]
            if dim_pairs:
                ...
                death = np.where(
                    np.isinf(death), birth + 1.0, death
                )
                diagrams[dim] = PersistenceDiagram(
...
class PAdicFiltration:
    ...
    def build(
        ...
        rips = RipsFiltration(
            ...
            max_edge_length=1.0,  # p-adic distances are <= 1
        )

        # For distance matrix input, we need to handle it specially
        return self._build_from_distance_matrix(dist_matrix)

    def _build_from_distance_matrix(
...
class PersistenceVectorizer:
    ...
    - persistence_image: 2D histogram weighted by persistence
    - landscape: Piecewise linear functions
    ...
    def transform(
        self, fingerprint: TopologicalFingerprint
    ) -> np.ndarray:
...
class ProteinTopologyEncoder(nn.Module):
    ...
    def __init__(
        ...
        if use_padic:
            self.padic = PAdicFiltration(
                prime=prime, max_dimension=max_dimension
            )
