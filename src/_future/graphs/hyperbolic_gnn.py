...
class PoincareOperations:
    ...
    The Poincare ball B^n_c = {x ∈ R^n : c||x||^2 < 1} is a model
    ...
    def mobius_scalar(self, r: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        """Mobius scalar multiplication: r ⊗ x.
...
class LorentzOperations:
    ...
    of a hyperboloid: H^n = {x ∈ R^{n+1} : <x,x>_L = -1/c, x_0 > 0}
...
class HyperbolicGraphConv(nn.Module):
    ...
    1. Aggregate neighbor features using hyperbolic mean
    2. Update with hyperbolic linear transformation
    """
    ...
    def forward(
        ...
        if self.use_attention:
            ...
            att = torch.zeros(n_nodes, self.heads, device=device)
...
class LorentzMLP(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [l for l in layers if isinstance(l, nn.Module)]
        )
        self.activations = [isinstance(l, nn.ReLU) for l in layers]

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass.

        Input x should be on the hyperboloid (first component is time).
        """
...
class HyboWaveNet(nn.Module):
    ...
    def __init__(
        ...
        self.scale_encoders = nn.ModuleList([
            HyperbolicLinear(in_channels, hidden_channels, curvature)
    ...
    def forward(
        ...
        for i, (wavelet_x, encoder, gnns) in enumerate(
            zip(wavelet_features, self.scale_encoders, self.scale_gnns)
        ):
            # Encode
            h = encoder(wavelet_x)
