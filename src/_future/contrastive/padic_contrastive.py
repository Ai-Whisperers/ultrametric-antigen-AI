...
from src.config.constants import TERNARY_BASE
...
class PAdicContrastiveLoss(nn.Module):
    ...
    def _compute_valuation(self, diff: torch.Tensor) -> torch.Tensor:
        ...
        valuation = torch.where(is_zero, torch.tensor(self.max_valuation, dtype=torch.float32, device=diff.device), valuation)
...
class MomentumContrastEncoder(nn.Module):
    ...
    def _copy_encoder(self, encoder: nn.Module) -> nn.Module:
        """Create a copy of the encoder for momentum update."""
        import copy
...
class PAdicPositiveSampler:
    ...
    def get_positive_candidates(
        ...
        valuation = torch.where(is_zero, torch.tensor(self.max_valuation, dtype=torch.float32, device=device), valuation)
