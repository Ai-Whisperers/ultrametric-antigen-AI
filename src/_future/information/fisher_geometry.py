    ...
    def step(self, closure: Optional[Callable] = None) -> Optional[torch.Tensor]:
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # Compute natural gradient: F^{-1} g ≈ g / (cov + damping)
...
class KFACOptimizer(Optimizer):
    ...
    as a Kronecker product: F ≈ A ⊗ S, where A is input covariance
    ...
    def _compute_natural_grad(
        ...
        # Natural gradient: (A^{-1} ⊗ S^{-1}) vec(G) = vec(S^{-1} G A^{-1})
        if grad.dim() == 2:
...
class InformationGeometricAnalyzer:
    ...
    def analyze_step(
        ...
        if self.track_eigenvalues and fisher_info.eigenvalues is not None:
            ...
            self.history["top_eigenvalues"].append(
                eigs[-self.n_eigenvalues:].cpu().numpy()
            )
            self.history["bottom_eigenvalues"].append(
                eigs[:self.n_eigenvalues].cpu().numpy()
            )
