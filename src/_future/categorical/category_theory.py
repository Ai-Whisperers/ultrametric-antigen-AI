...
class CategoricalLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.input_type.is_compatible(x):
            raise TypeError(
                f"Input shape {x.shape} incompatible with {self.input_type}"
            )
        ...
        if not self.output_type.is_compatible(output):
            raise TypeError(
                f"Output shape {output.shape} incompatible with {self.output_type}"
            )
    ...
    def compose(self, other: "CategoricalLayer") -> "CategoricalLayer":
        ...
        if self.output_type != other.input_type:
            raise TypeError(
                f"Cannot compose: output {self.output_type} != input {other.input_type}"
            )
...
class NaturalTransformation(Generic[A, B]):
    ...
    def verify_naturality(
        self, morphism: Morphism, test_input: torch.Tensor
    ) -> bool:
...
class LinearLens(ParametricLens):
    ...
    def __init__(self, in_features: int, out_features: int):
        ...
        self.params = nn.Parameter(
            torch.randn(out_features, in_features + 1) * 0.01
        )
...
class ProductCategory:
    ...
    @staticmethod
    def product_type(
        type1: TensorType, type2: TensorType
    ) -> TensorType:
    ...
    def product_morphism(
        morph1: Morphism, morph2: Morphism
    ) -> Morphism:
...
class MonoidalCategory:
    ...
    @staticmethod
    def tensor_product(
        layer1: CategoricalLayer, layer2: CategoricalLayer
    ) -> CategoricalLayer:
        ...
        """
        # Product types
        input_type = ProductCategory.product_type(
            layer1.input_type, layer2.input_type
        )
        output_type = ProductCategory.product_type(
            layer1.output_type, layer2.output_type
        )
...
class StringDiagram:
    ...
    def connect(
        self, from_box: int, from_port: int, to_box: int, to_port: int
    ):
...
class ResidualOptic(Optic):
    ...
    def __init__(self, layer: nn.Module):
        ...
        def backward_fn(
            residual: torch.Tensor, dy: torch.Tensor
        ) -> torch.Tensor:
...
class AttentionOptic(Optic):
    ...
    def __init__(
        ...
        def forward_fn(
            ...
            q = q.view(batch, seq_len, self.n_heads, self.head_dim).transpose(
                1, 2
            )
            k = k.view(batch, seq_len, self.n_heads, self.head_dim).transpose(
                1, 2
            )
            v = v.view(batch, seq_len, self.n_heads, self.head_dim).transpose(
                1, 2
            )
        ...
        def backward_fn(
            attn: torch.Tensor, dy: torch.Tensor
        ) -> torch.Tensor:
            return dy
    ...
    def add_layer(self, layer: CategoricalLayer) -> "CategoricalNetwork":
        ...
        if self.layers:
            ...
            if last_layer.output_type != layer.input_type:
                raise TypeError(
                    f"Type mismatch: {last_layer.output_type} != {layer.input_type}"
                )
