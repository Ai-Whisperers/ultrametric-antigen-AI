...
class TropicalPolynomial:
    ...
    def from_linear(cls, weights: np.ndarray, bias: float) -> "TropicalPolynomial":
        ...
        poly = cls()
        exponents = tuple(int(w) if w == int(w) else 1 for w in weights)
    ...
    def relu(cls, input_poly: "TropicalPolynomial") -> "TropicalPolynomial":
        ...
        zero_poly = cls()
        zero_poly.add_monomial(TropicalMonomial(0.0, (0,) * len(input_poly.monomials[0].exponents if input_poly.monomials else ())))
...
class TropicalNNAnalyzer:
    ...
    def extract_tropical_polynomial(
        ...
        if len(self.layers) == 1:
            ...
            poly.add_monomial(TropicalMonomial(
