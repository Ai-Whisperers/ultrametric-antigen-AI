...
class SpinGlassLandscape(nn.Module):
    ...
    def __init__(
        ...
        self.field = nn.Parameter(
            torch.randn(n_sites, n_states) * field_scale, requires_grad=False
        )
    ...
    def local_field(
        self, configuration: torch.Tensor, site: int
    ) -> torch.Tensor:
    ...
    def forward(
        self, configuration: torch.Tensor, temperature: float = 1.0
    ) -> torch.Tensor:
...
class ReplicaExchange:
    ...
    def _create_temp_ladder(self) -> torch.Tensor:
        ...
        if self.temp_schedule == "geometric":
            ...
            temps = torch.linspace(
                self.temp_min, self.temp_max, self.n_replicas
            ).tolist()
    ...
    def sample(
        ...
        if initial_configs is None:
            configs = [
                torch.randint(n_states, (n_sites,)) for _ in range(self.n_replicas)
            ]
        else:
        ...
        for iteration in range(n_samples * sample_interval):
            ...
            for r in range(self.n_replicas):
                configs[r], _ = self._metropolis_sweep(
                    landscape, configs[r], self.temperatures[r].item()
                )
            ...
            if iteration % sample_interval == 0:
                ...
                energies.append(
                    landscape.energy(configs[0].unsqueeze(0)).item()
                )

        # Compute exchange rate
        exchange_rate = (
            n_exchanges_accepted / n_exchanges_attempted
            if n_exchanges_attempted > 0
            else 0.0
        )
            ...
            for ii, ci in enumerate(active_clusters):
                for jj, cj in enumerate(active_clusters[ii + 1:], ii + 1):
                    dist = self._cluster_distance(
                        D, cluster_members[ci], cluster_members[cj]
                    )
                    if dist < min_dist:
                        min_dist = dist
                        best_i, best_j = ii, jj
...
class ParisiOverlapAnalyzer:
    ...
    def compute_overlap(
        self, config1: torch.Tensor, config2: torch.Tensor
    ) -> float:
    ...
    def overlap_distribution(
        ...
        hist, bin_edges = torch.histogram(
            overlaps, bins=self.n_bins, range=(-1.0, 1.0)
        )
    ...
    def analyze_phase(
        self, samples: torch.Tensor
    ) -> Dict[str, Union[float, str, torch.Tensor]]:
...
class BoltzmannMachine(nn.Module):
    ...
    def forward(
        self, v: torch.Tensor, n_samples: int = 1
    ) -> torch.Tensor:
