...
def phase_1_2_padic_balls(geodesic_matrix, codons, amino_acids):
    ...
    for aa in sorted(aa_to_indices.keys()):
        ...
        print(f"    {aa} ({n_codons} codons): eps_in={epsilon_within:.4f}, "
...
def visualize_padic_balls(geodesic_matrix, codons, amino_acids, codon_embeddings, output_dir):
    ...
    for i, (x, y) in enumerate(coords_2d):
        aa = amino_acids[i]
        ax1.scatter(x, y, c=[aa_to_color[aa]], s=80, alpha=0.7, edgecolors='black', linewidths=0.5)
    ...
    for deg in sorted(set(degeneracies)):
        mask = degeneracies == deg
        ax2.scatter([deg] * mask.sum(), radii[mask], alpha=0.6, s=50, label=f'Deg {deg}')
    ...
    ax4.hist(within_distances, bins=30, alpha=0.7, label=f'Within AA (n={len(within_distances)})', density=True)
    ax4.hist(between_distances, bins=30, alpha=0.7, label=f'Between AA (n={len(between_distances)})', density=True)
    ax4.axvline(np.mean(within_distances), color='blue', linestyle='--', label=f'Within mean: {np.mean(within_distances):.3f}')
    ax4.axvline(np.mean(between_distances), color='orange', linestyle='--', label=f'Between mean: {np.mean(between_distances):.3f}')
