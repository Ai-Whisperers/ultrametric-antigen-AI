...
GENETIC_CODE_DEGENERACY = [1, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 3, 4, 4, 4, 4, 4, 6, 6, 6]
...
def find_optimal_64_greedy(embeddings, valuations, n_clusters=21):
    ...
    for trial in range(200):
        ...
        for cluster_size in target_pattern:
            ...
            candidates = [sorted_indices[i] for i in range(min(1000, len(sorted_indices)))
...
def visualize_configuration(indices, labels, embeddings, valuations, output_dir):
    ...
    for i, c in enumerate(unique_labels):
        ...
        ax1.scatter(coords_2d[mask, 0], coords_2d[mask, 1],
                   c=[colors[i]], s=80, alpha=0.7, label=f'C{c}', edgecolors='black')
    ...
    for i, c in enumerate(unique_labels):
        mask = np.array(labels) == c
        ax2.scatter(selected_vals[mask], selected_radii[mask],
...
def main():
    ...
    if best_method[1] is not None:
        ...
        if selected_vals_mean > all_vals_mean + 0.5:
            ...
            print(f"  This suggests the genetic code maps to 'deeper' algebraic operations!")
