...
DEGENERACY_PATTERN = sorted([
    6, 6, 6,        # Leu, Ser, Arg (6 codons each)
    4, 4, 4, 4, 4,  # Val, Pro, Thr, Ala, Gly (4 codons each)
    3, 3,           # Ile, stop codons (3 each)
    2, 2, 2, 2, 2, 2, 2, 2, 2,  # Phe, Tyr, His, Gln, Asn, Lys, Asp, Glu, Cys (2 each)
    1, 1            # Met, Trp (1 each)
], reverse=True)
...
def find_natural_positions(embeddings, target_pattern=DEGENERACY_PATTERN):
    ...
    for cluster_idx, (cluster_id, cluster_size) in enumerate(sorted_clusters):
        ...
        # Distance to center
        dists_to_center = [poincare_distance(embeddings[i], cluster_center)
...
def refine_positions(embeddings, indices, labels, iterations=5):
    ...
    for iteration in range(iterations):
        ...
        for i, idx in enumerate(best_indices):
            ...
            cluster_indices = [best_indices[j] for j in range(len(best_indices))
            ...
            candidates = [j for j in all_indices - selected_set
...
def main():
    ...
    output = {
        ...
        'clusters': {
            str(label): positions
            for label, positions in sorted_clusters
        },
