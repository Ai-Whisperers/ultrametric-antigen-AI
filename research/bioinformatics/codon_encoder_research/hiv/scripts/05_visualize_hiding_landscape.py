...
PROTEIN_GENES = {
    'Gag_MA_p17': 'gag', 'Gag_CA_p24': 'gag', 'Gag_NC_p7': 'gag',
    'Pol_PR': 'pol', 'Pol_RT': 'pol', 'Pol_IN': 'pol',
    'Env_gp120': 'env', 'Env_gp41': 'env',
    'Tat': 'regulatory', 'Rev': 'regulatory',
    'Nef': 'accessory', 'Vif': 'accessory', 'Vpr': 'accessory', 'Vpu': 'accessory',
}
...
def plot_distance_heatmap(results: Dict, output_dir: Path):
    ...
    ax.set_title('HIV Protein Distance Matrix in Hiding Space\n(3-adic Poincaré Geometry)', fontsize=12)

    plt.tight_layout()
    plt.savefig(output_dir / 'hiv_hiding_distance_matrix.png', dpi=FIGURE_DPI, bbox_inches='tight')
    plt.close()
...
def plot_vulnerability_network(results: Dict, output_dir: Path):
    ...
    if vuln_edges:
        ...
        nx.draw_networkx_edges(G, pos, edgelist=vuln_edges, edge_color='red',
                              style='dashed', width=1.5, alpha=0.6, ax=ax)

    # Close connections (green, solid)
    if close_edges:
        nx.draw_networkx_edges(G, pos, edgelist=close_edges, edge_color='green',
                              style='solid', width=2, alpha=0.8, ax=ax)
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], color='red', linestyle='--', label='Vulnerability zones (d > 2.0)'),
        plt.Line2D([0], [0], color='green', linestyle='-', linewidth=2, label='Clustered hiding (d < 1.0)'),
    ]
    ax.legend(handles=legend_elements, loc='upper left', fontsize=9)

    ax.set_title('HIV Protein Hiding Network\nRed dashed = Vulnerability zones | Green solid = Shared hiding',
                fontsize=12)
    ax.axis('off')

    plt.tight_layout()
    plt.savefig(output_dir / 'hiv_vulnerability_network.png', dpi=FIGURE_DPI, bbox_inches='tight')
    plt.close()
...
def plot_hierarchy_distribution(results: Dict, output_dir: Path):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'hiv_hiding_distribution.png', dpi=FIGURE_DPI, bbox_inches='tight')
    plt.close()
...
def plot_evolutionary_space(results: Dict, output_dir: Path):
    ...
    level_colors = {'protein': '#3498DB', 'signaling': '#E74C3C',
                   'glycan': '#2ECC71', 'peptide': '#9B59B6'}

    for level, data in by_level.items():
        norm = data['centroid_norm']
        # Place at angle based on level
        angles = {'protein': 0, 'signaling': np.pi/2, 'glycan': np.pi, 'peptide': 3*np.pi/2}
        angle = angles.get(level, 0)

        x = norm * np.cos(angle)
        y = norm * np.sin(angle)

        ax1.scatter([x], [y], s=200, c=level_colors[level], label=f'{level} (n={data["n_proteins"]})',
                   edgecolors='black', linewidth=2, zorder=5)
        ax1.annotate(level, (x, y), xytext=(10, 10), textcoords='offset points',
    ...
    ax1.set_title('HIV Hiding in Poincaré Space\n(center = flexibility, boundary = constraint)', fontsize=11)
    ...
    # Add reference lines
    ax2.axhline(y=0.3, color='green', linestyle='--', alpha=0.7, label='Flexible zone (< 0.3)')
    ax2.axhline(y=0.7, color='red', linestyle='--', alpha=0.7, label='Constrained zone (> 0.7)')

    # Add overall centroid line
    ax2.axhline(y=geometry['overall_centroid_norm'], color='black', linestyle='-',
               linewidth=2, label=f'Overall centroid ({geometry["overall_centroid_norm"]:.3f})')

    ax2.set_ylabel('Centroid Norm (distance from center)', fontsize=11)
    ax2.set_xlabel('Hierarchy Level', fontsize=11)
    ax2.set_title('Evolutionary Flexibility by Level\n(lower = more flexible)', fontsize=11)
    ax2.legend(fontsize=9)
    ax2.set_ylim(0, 1.0)

    for bar, norm in zip(bars, norms):
        ax2.text(bar.get_x() + bar.get_width()/2, norm + 0.02,
                f'{norm:.3f}', ha='center', va='bottom', fontsize=9, fontweight='bold')

    plt.tight_layout()
    plt.savefig(output_dir / 'hiv_evolutionary_space.png', dpi=FIGURE_DPI, bbox_inches='tight')
    plt.close()
...
def plot_integrase_isolation(results: Dict, output_dir: Path):
    ...
    ax.set_title('INTEGRASE ISOLATION: Distance from All Other HIV Proteins\n'
                '(Largest distances = weakest hiding connections)', fontsize=12)
    ax.legend(fontsize=10)

    # Mark all as vulnerability zones
    for bar, dist in zip(bars, dists):
        color = 'red' if dist > 2.0 else 'black'
        ax.text(dist + 0.05, bar.get_y() + bar.get_height()/2,
               f'{dist:.2f}', ha='left', va='center', fontsize=9, color=color,
               fontweight='bold' if dist > 3.0 else 'normal')

    plt.tight_layout()
    plt.savefig(output_dir / 'hiv_integrase_isolation.png', dpi=FIGURE_DPI, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print(f"  Loaded: {results['metadata']['total_proteins']} proteins, "
