...
def add_goldilocks_zones(ax, ymin=0, ymax=1, alpha=0.15):
    ...
    # Too low (<15%)
    ax.axvspan(0, 0.15, color=PALETTE['goldilocks_low'], alpha=alpha, label='Self (no response)')
    # Goldilocks (15-30%)
    ax.axvspan(0.15, 0.30, color=PALETTE['goldilocks_zone'], alpha=alpha, label='Modified self (autoimmunity)')
    # Too high (>30%)
    ax.axvspan(0.30, 1.0, color=PALETTE['goldilocks_high'], alpha=alpha, label='Foreign (cleared)')
