...
def create_epitope_distance_heatmap():
    ...
    for i, imm in enumerate(immunodominant):
        if imm:
            ax1.text(-0.7, i, '●', fontsize=12, color=PALETTE['immunodominant'],
                    ha='center', va='center')
    ...
    scatter = ax2.scatter(shifts, js_divergences, c=colors, s=sizes,
                         edgecolors='white', linewidths=1.5, alpha=0.8)

    # Add labels
    for i, ep in enumerate(epitopes):
        ax2.annotate(ep, (shifts[i], js_divergences[i]),
...
def create_comparison_heatmap():
    ...
    metric_labels = ['Centroid Shift', 'Relative Shift', 'JS Divergence', 'Entropy Change']
    ...
    bars1 = ax1.bar(x - width/2, imm_means, width, yerr=imm_stds, capsize=4,
                   label='Immunodominant', color=PALETTE['immunodominant'], alpha=0.8)
    bars2 = ax1.bar(x + width/2, sil_means, width, yerr=sil_stds, capsize=4,
                   label='Silent', color=PALETTE['silent'], alpha=0.8)
    ...
    bars = ax2.barh(metric_labels, [-np.log10(p) for p in p_values], color=colors, alpha=0.8)

    ax2.axvline(-np.log10(0.05), color='red', linestyle='--', linewidth=1.5, label='p = 0.05')
    ax2.axvline(-np.log10(0.01), color='orange', linestyle='--', linewidth=1.5, label='p = 0.01')
    ...
    ax3 = axes[1, 0]
    colors_es = ['#D32F2F' if abs(es) > 0.8 else '#FF9800' if abs(es) > 0.5 else '#4CAF50'
    ...
    # Add interpretation
    ax3.text(0.95, 0.05, 'Large effect: |d| > 0.8\nMedium: 0.5-0.8\nSmall: < 0.5',
