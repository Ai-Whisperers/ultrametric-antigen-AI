...
def validate_against_ground_truth(
    ...
    feature_names = [
        'centroid_norm', 'cluster_diversity', 'mean_pairwise_dist',
        'mean_entropy', 'r_count', 'r_fraction', 'r_to_q_effect',
        'centroid_shift', 'relative_shift', 'goldilocks_score'
    ]
...
def generate_validation_plots(results: Dict, output_dir: Path):
    ...
    significant_features = [
        (f, d) for f, d in results['statistical_tests'].items()
        if d['significant']
    ]
    ...
    if significant_features:
        ...
        bars1 = ax.bar(x - width/2, imm_means, width, yerr=imm_stds,
                       label='Immunodominant', color='coral', alpha=0.7, capsize=3)
        bars2 = ax.bar(x + width/2, sil_means, width, yerr=sil_stds,
                       label='Silent', color='steelblue', alpha=0.7, capsize=3)
    ...
    out_gold = [gold_val['imm_total'] - gold_val['imm_in_goldilocks'],
                gold_val['sil_total'] - gold_val['sil_in_goldilocks']]

    x = np.arange(len(categories))
    width = 0.35

    bars1 = ax.bar(x - width/2, in_gold, width, label='In Goldilocks', color='gold', alpha=0.8)
    bars2 = ax.bar(x + width/2, out_gold, width, label='Outside Goldilocks', color='gray', alpha=0.5)

    ax.set_xticks(x)
    ax.set_xticklabels(categories)
    ax.set_ylabel('Count', fontsize=10)
    ax.set_title(f"Goldilocks Zone Validation\n(Fisher p={gold_val['fisher_p_value']:.4f}, OR={gold_val['fisher_odds_ratio']:.2f})",
    ...
    for i, (ing, outg) in enumerate(zip(in_gold, out_gold)):
        ...
        ax.text(i - width/2, ing + 0.5, f'{pct:.0f}%', ha='center', fontsize=10, fontweight='bold')

    # 5. Correlation with ACPA reactivity
    ax = axes[1, 1]

    # Best correlating feature
    best_corr_feature = max(results['correlation_results'].items(),
                           key=lambda x: abs(x[1]['spearman_r']))[0]
    ...
    ax.scatter(feature_vals, acpa_vals, alpha=0.6, s=50,
...
"""
    ...
    plt.suptitle('Hierarchical Model Validation: Proving Disruption Prediction',
                fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_dir / 'hierarchical_validation.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # ACPA correlation
    print("\n5. CORRELATION WITH ACPA REACTIVITY:")
    for f, c in sorted(results['correlation_results'].items(),
                       key=lambda x: -abs(x[1]['spearman_r']))[:3]:
    ...
    proof_level = 0
    if best_auc > 0.6: proof_level += 1
    if combined_auc > 0.65: proof_level += 1
    if any_significant: proof_level += 1
    if goldilocks_significant: proof_level += 1
