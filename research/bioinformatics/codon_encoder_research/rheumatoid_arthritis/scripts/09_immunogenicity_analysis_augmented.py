...
def analyze_epitope(epitope: dict, protein_info: dict, encoder, cluster_centers: np.ndarray, device='cpu') -> dict:
...
def statistical_comparison(immunodominant: list, silent: list, metric_name: str) -> dict:
    ...
    pooled_std = np.sqrt(((len(imm_values)-1)*np.var(imm_values, ddof=1) +
...
def main():
    ...
    basic_metrics = ['embedding_norm', 'cluster_homogeneity', 'mean_neighbor_distance', 'boundary_potential']
    comparisons = {}
    ...
    for metric in basic_metrics:
        ...
        if comparison:
            ...
            print(f"\n{metric.upper()}:")
            print(f"  Immunodominant: {comparison['immunodominant_mean']:.4f} +/- {comparison['immunodominant_std']:.4f} (n={comparison['immunodominant_n']})")
