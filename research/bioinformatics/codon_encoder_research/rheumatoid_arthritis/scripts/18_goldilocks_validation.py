...
def compute_goldilocks_boundaries(imm_values: List[float], silent_values: List[float]) -> Dict:
    ...
    pooled_std = np.sqrt(((len(imm)-1)*imm_std**2 + (len(silent)-1)*silent_std**2) /
...
def plot_goldilocks_zones(results: Dict, output_dir: Path):
    ...
    metrics = ['entropy_change', 'centroid_shift', 'js_divergence', 'relative_entropy_change']
    titles = ['Entropy Change', 'Centroid Shift', 'JS Divergence', 'Relative Entropy Change']
    ...
    for ax, metric, title in zip(axes.flat, metrics, titles):
        ...
        ], labels=['Immunodominant', 'Silent'], patch_artist=True)
        ...
        # Goldilocks zone
        ax.axhspan(zone['lower_bound'], zone['upper_bound'],
    ...
    plt.suptitle('Goldilocks Zone Analysis: 3-adic Encoder (V5.11.3)',
...
def plot_entropy_detail(results: Dict, output_dir: Path):
    ...
    ax = axes[0]
    ax.hist(imm_vals, bins=15, alpha=0.6, color='#e53935', label='Immunodominant', density=True)
    ax.hist(silent_vals, bins=15, alpha=0.6, color='#1e88e5', label='Silent', density=True)
    ax.axvline(0, color='black', linestyle='-', lw=1, label='No change')
    ax.axvline(zone['center'], color='gold', linestyle='--', lw=2, label='Goldilocks center')
    ...
    imm_epitopes = [(e['epitope_id'], e['mean_entropy_change'], e['acpa_reactivity'])
                    for e in all_epitopes if e['immunodominant']]
    silent_epitopes = [(e['epitope_id'], e['mean_entropy_change'], e['acpa_reactivity'])
                       for e in all_epitopes if not e['immunodominant']]
    ...
    ax.barh(y_imm, [e[1] for e in imm_epitopes], color='#e53935', alpha=0.7, label='Immunodominant')
    ax.barh(y_silent, [e[1] for e in silent_epitopes], color='#1e88e5', alpha=0.7, label='Silent')

    ax.axvline(0, color='black', linestyle='-', lw=1)
    ax.axvspan(zone['lower_bound'], zone['upper_bound'], alpha=0.2, color='gold')

    ax.set_yticks(list(y_imm) + list(y_silent))
    ax.set_yticklabels([e[0][:10] for e in imm_epitopes] + [e[0][:10] for e in silent_epitopes], fontsize=7)
...
def main():
    ...
    json_results['epitope_details'] = [
        {k: v for k, v in e.items() if k != 'per_r_metrics'}
        for e in epitope_metrics
    ]
