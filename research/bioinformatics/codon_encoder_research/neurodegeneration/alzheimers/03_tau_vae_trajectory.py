...
def compare_trajectories(trajectories: List[Dict]) -> Dict:
    ...
    for traj in trajectories:
        ...
        comparison['max_steps'].append({
            'site': summary['max_step_site'],
            'distance': summary['max_step']
        })
...
def plot_trajectory_2d(trajectory: Dict, output_path: Path, title: str = None):
    ...
    scatter = ax1.scatter(coords_2d[:, 0], coords_2d[:, 1],
                          c=n_phospho, cmap='Reds', s=100, edgecolors='black')

    # Mark start and end
    ax1.scatter(coords_2d[0, 0], coords_2d[0, 1], c='green', s=200,
                marker='*', label='Healthy', zorder=5, edgecolors='black')
    ax1.scatter(coords_2d[-1, 0], coords_2d[-1, 1], c='red', s=200,
                marker='X', label='Hyperphosphorylated', zorder=5, edgecolors='black')

    # Annotate some key points
    for i, step in enumerate(steps):
        if step['new_site'] in [202, 262, 396]:  # Key epitope sites
            ax1.annotate(f"pS{step['new_site']}",
    ...
    ax2.plot(range(len(steps)), cumulative_distances, 'r-o', label='Cumulative Distance')
...
def plot_poincare_disk(trajectory: Dict, output_path: Path):
    ...
    scatter = ax.scatter(centroids[:, 0], centroids[:, 1],
                         c=n_phospho, cmap='plasma', s=100, edgecolors='black')

    # Mark healthy center
    ax.scatter(centroids[0, 0], centroids[0, 1], c='green', s=300,
               marker='*', zorder=5, edgecolors='black', label='Healthy')

    # Mark hyperphosphorylated
    ax.scatter(centroids[-1, 0], centroids[-1, 1], c='red', s=300,
               marker='X', zorder=5, edgecolors='black', label='AD')
...
def main():
    ...
    mtbr_sites = [pos for pos in PATHOLOGICAL_ORDER
    ...
    traj_mtbr = compute_full_trajectory(
        mtbr_seq,
        mtbr_positions,
        encoder,
        "MTBR Only"
    ...
    for i in range(RANDOM_ORDERS):
        ...
        print(f"  Random {i+1}: path length = {traj['summary']['total_distance']:.4f}, "
    ...
    for n in [0, 2, 4, 6, 8, 10, len(PATHOLOGICAL_ORDER)]:
        if n < len(steps):
            ...
            print(f"  {n:2d} phospho ({stage:12s}): distance = {step['cumulative_distance']:.4f}, "
    ...
    try:
        ...
        plot_poincare_disk(
            traj_full,
            viz_dir / "poincare_trajectory.png"
        )
