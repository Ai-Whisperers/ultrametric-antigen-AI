...
def plot_poincare_disk_2d(data, encoder, output_path):
    ...
    ax1.set_title('Epitope Centroids in Poincaré Disk\n(by Immunodominance)', fontsize=14, fontweight='bold')

    # Legend
    imm_patch = mpatches.Patch(color=COLORS['immunodominant'], label=f'Immunodominant (n={sum(1 for l in data["labels"] if l=="immunodominant")})')
    sil_patch = mpatches.Patch(color=COLORS['silent'], label=f'Silent (n={sum(1 for l in data["labels"] if l=="silent")})')
    ...
    for i, (x, y) in enumerate(centroids_2d):
        ...
        ax2.scatter(x, y, c=[color], s=100, alpha=0.7, marker=marker, edgecolors='white', linewidth=0.5)

    ax2.set_xlim(-1.15, 1.15)
    ax2.set_ylim(-1.15, 1.15)
    ax2.set_xlabel('PC1', fontsize=12)
    ax2.set_ylabel('PC2', fontsize=12)
    ax2.set_title('Epitope Centroids in Poincaré Disk\n(by Protein)', fontsize=14, fontweight='bold')
...
def plot_3d_embedding_space(data, encoder, output_path):
    ...
    ax1.scatter(centroids_3d[imm_mask, 0], centroids_3d[imm_mask, 1], centroids_3d[imm_mask, 2],
                c=COLORS['immunodominant'], s=80, alpha=0.7, label='Immunodominant', edgecolors='white')
    ax1.scatter(centroids_3d[sil_mask, 0], centroids_3d[sil_mask, 1], centroids_3d[sil_mask, 2],
                c=COLORS['silent'], s=80, alpha=0.7, label='Silent', edgecolors='white')
    ...
    scatter = ax2.scatter(centroids_3d[:, 0], centroids_3d[:, 1], centroids_3d[:, 2],
                          c=acpa, cmap='RdYlBu_r', s=80, alpha=0.7, edgecolors='white')
    ...
    for i, protein in enumerate(proteins):
        ...
        ax3.scatter(centroids_3d[mask, 0], centroids_3d[mask, 1], centroids_3d[mask, 2],
                    c=[colors[i]], s=80, alpha=0.7, label=protein, edgecolors='white')
    ...
    plt.suptitle('3D Epitope Embedding Space (PCA Projection)', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_path, dpi=DPI, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def plot_3d_poincare_ball(data, encoder, output_path):
    ...
    ax.scatter(centroids_3d[imm_mask, 0], centroids_3d[imm_mask, 1], centroids_3d[imm_mask, 2],
               c=COLORS['immunodominant'], s=120, alpha=0.8, label='Immunodominant',
               edgecolors='white', linewidth=0.5)
    ax.scatter(centroids_3d[sil_mask, 0], centroids_3d[sil_mask, 1], centroids_3d[sil_mask, 2],
               c=COLORS['silent'], s=120, alpha=0.8, label='Silent',
               edgecolors='white', linewidth=0.5)
    ...
    ax.set_title('Epitope Centroids in Poincaré Ball\n(3D PCA Projection)', fontsize=14, fontweight='bold')
...
def plot_statistical_comparisons(results_path, output_path):
    ...
    for idx, (metric_key, metric_name) in enumerate(metrics):
        ...
        bars = ax.bar(x, means, yerr=stds, color=colors, alpha=0.7, capsize=5, edgecolor='white', linewidth=2)
        ...
        ax.annotate(sig_text, xy=(0.5, max_height * 1.05), fontsize=14, ha='center', fontweight='bold')
    ...
    plt.suptitle('Statistical Comparisons: Immunodominant vs Silent Epitopes',
...
def plot_entropy_change_detail(results_path, output_path):
    ...
    # Plot 1: Box plot
    ax1 = axes[0]
    bp = ax1.boxplot([imm_entropy, sil_entropy], labels=['Immunodominant', 'Silent'],
    ...
    ax2.hist(imm_entropy, bins=bins, alpha=0.6, color=COLORS['immunodominant'],
             label=f'Immunodominant (n={len(imm_entropy)})', edgecolor='white')
    ax2.hist(sil_entropy, bins=bins, alpha=0.6, color=COLORS['silent'],
             label=f'Silent (n={len(sil_entropy)})', edgecolor='white')
    ax2.axvline(x=0, color='gray', linestyle='--', linewidth=1)
    ax2.axvline(x=np.mean(imm_entropy), color=COLORS['immunodominant'], linestyle='-', linewidth=2)
    ax2.axvline(x=np.mean(sil_entropy), color=COLORS['silent'], linestyle='-', linewidth=2)
    ...
    ax3.scatter(x_imm, imm_entropy, c=COLORS['immunodominant'], alpha=0.6, s=60, edgecolors='white')
    ax3.scatter(x_sil, sil_entropy, c=COLORS['silent'], alpha=0.6, s=60, edgecolors='white')

    # Mean bars
    ax3.hlines(np.mean(imm_entropy), -0.2, 0.2, colors=COLORS['immunodominant'], linewidth=3)
    ...
    # Add annotation
    ax3.annotate(f'Mean: +{np.mean(imm_entropy):.3f}', xy=(0, np.mean(imm_entropy) + 0.03),
                 fontsize=10, ha='center', color=COLORS['immunodominant'], fontweight='bold')
    ax3.annotate(f'Mean: {np.mean(sil_entropy):.3f}', xy=(1, np.mean(sil_entropy) - 0.03),
                 fontsize=10, ha='center', color=COLORS['silent'], fontweight='bold')

    plt.suptitle('KEY FINDING: Entropy Preservation Upon Citrullination\n'
...
def plot_3d_tsne(data, output_path):
    ...
    ax1.scatter(centroids_tsne[imm_mask, 0], centroids_tsne[imm_mask, 1], centroids_tsne[imm_mask, 2],
                c=COLORS['immunodominant'], s=100, alpha=0.7, label='Immunodominant', edgecolors='white')
    ax1.scatter(centroids_tsne[sil_mask, 0], centroids_tsne[sil_mask, 1], centroids_tsne[sil_mask, 2],
                c=COLORS['silent'], s=100, alpha=0.7, label='Silent', edgecolors='white')
    ...
    acpa = np.array(data['acpa'])
    scatter = ax2.scatter(centroids_tsne[:, 0], centroids_tsne[:, 1], centroids_tsne[:, 2],
                          c=acpa, cmap='RdYlBu_r', s=100, alpha=0.7, edgecolors='white')
    ...
    plt.suptitle('t-SNE 3D Embedding Visualization', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_path, dpi=DPI, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: {output_path}")
...
def plot_cluster_distribution_heatmap(data, encoder, output_path):
    ...
    ax1.set_title(f'Immunodominant Epitopes (n={len(imm_distributions)})\nCluster Distribution',
    ...
    ax2.set_title(f'Silent Epitopes (n={len(sil_distributions)})\nCluster Distribution',
                  fontsize=12, fontweight='bold')
