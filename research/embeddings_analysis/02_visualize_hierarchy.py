...
def plot_valuation_radius_comparison():
    ...
    for variant in variants:
        ...
        ax1.plot(valuations, radii, 'o-', color=colors[variant],
                 label=labels[variant], linewidth=2, markersize=8)

    # Add expected trend line
    expected = [0.85 - 0.03*v for v in range(9)]
    ax1.plot(range(9), expected, '--', color='gray', alpha=0.5,
             label='Expected (monotonic decrease)', linewidth=1.5)
    ...
    ax1.annotate('Hierarchy\nInversion', xy=(6, 0.85), fontsize=10,
                 color='red', ha='center')
    ...
    v5_11_radii = [results['v5_11']['embedding_analysis']['per_valuation_stats'].get(str(v), {}).get('mean_radius', 0)
                  for v in valuations]
    overnight_radii = [results['v5_11_overnight']['embedding_analysis']['per_valuation_stats'].get(str(v), {}).get('mean_radius', 0)
                       for v in valuations]
    ...
    bars2 = ax2.bar(x + width/2, overnight_radii, width, label='v5.11 Overnight', color='#e74c3c')
    ...
    for i in range(4, 9):
        if i < len(x):
            ax2.annotate('', xy=(x[i] + width/2, overnight_radii[i]),
...
def plot_hierarchy_delta():
    ...
    # Add annotations
    ax.annotate('Overnight radius\nTOO LARGE\n(should be smaller)', xy=(5, 0.15),
               fontsize=10, ha='center', color='#e74c3c')
    ax.annotate('Correct\n(overnight smaller)', xy=(1, -0.05),
               fontsize=10, ha='center', color='#2ecc71')
